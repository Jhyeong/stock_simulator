...
const KospiTop = (props) => { 
    ...
    const stockListColumns = [
        ...
        { field: 'currentAmount', headerName: '현재가'      , flex: 1 },
        { field: 'changedPrice' , headerName: '전일비'      , flex: 1, cellClassName: (params) => (parseInt(params.value) > 0 ? "up" : "down")},
        { field: 'changedRatio' , headerName: '등락률'      , flex: 1, cellClassName: (params) => ((params.value.substr(0, 1)) == "+" ? "up" : "down")}
    ];
            ...
            <Card className={classes.cardArea}>
                <CardContent>
                    ...
                    <div>
                        ...
                        <FormLabel component="legend">KOSPI 시총 상위 50</FormLabel>
 ...
 * @returns 
 */
const callKospiTop50List = async (type) => {
    let stockList;
    await axios({url:process.env.NEXT_PUBLIC_API_URL + "/api/KospiTop50List?type=" + type}).then(response => {
...
export async function getServerSideProps(){
    const stockDataList = await callKospiTop50List();                    //주식리스트
